...
def _render(filename, source, line, col, kind, msg, hint=None, length=1, code=None):
    src_line = _get_line(filename, source, line)
    code_str = f' [{code}]' if code else ''
    header = _red(_bold(f'{kind}Error')) + code_str + f' at {_cyan(filename)}:{_yellow(str(line))}:{_yellow(str(col))}'
    parts = [header]
    if src_line:
...
class FeelError(Exception):
    """Error utama Feel — selalu carry posisi sumber + machine-parseable code."""

    def __init__(self, message, *, filename='<input>', line=1, col=1, hint=None,
                 kind='Syntax', length=1, source=None, code=None):
        ...
        self.source = source
        self.code = code
        super().__init__(_render(filename, source, line, col, kind, message, hint, length, code))

    def to_dict(self):
        """Machine-readable form — for AI tools, JSON output, etc."""
        return {
            'error': f'{self.kind}Error',
            'code': self.code,
            'file': self.filename,
            'line': self.line,
            'col': self.col,
    ...
    @classmethod
    def syntax(cls, token, message, hint=None, filename='<input>', source=None, code='E_SYNTAX'):
        """Build syntax error from Token."""
        ...
        return cls(message, filename=filename, line=line, col=col,
                   hint=hint, kind='Syntax', length=length, source=source, code=code)

    @classmethod
    def syntax_at(cls, filename, source, line, col, message, hint=None, length=1, code='E_SYNTAX'):
        return cls(message, filename=filename, line=line, col=col,
                   hint=hint, kind='Syntax', length=length, source=source, code=code)

    @classmethod
    def runtime(cls, node, message, hint=None, filename='<input>', source=None, code='E_RUNTIME'):
        """Build runtime error from AST node (which already carries line/col)."""
        line = getattr(node, 'line', 1) or 1
        col = getattr(node, 'col', 1) or 1
        return cls(message, filename=filename, line=line, col=col,
                   hint=hint, kind='Runtime', length=1, source=source, code=code)
    ...
    def name_error(cls, node, name, filename='<input>', source=None, similar=None):
        ...
        return cls.runtime(node, f"'{name}' is not defined", hint=hint,
                           filename=filename, source=source, code='E_UNDEFINED_NAME')

    @classmethod
    def type_error(cls, node, message, hint=None, filename='<input>', source=None, code='E_TYPE'):
        return cls(message, filename=filename,
                   line=getattr(node, 'line', 1) or 1,
                   col=getattr(node, 'col', 1) or 1,
                   hint=hint, kind='Type', length=1, source=source, code=code)
